---
name: code-review
description: "Review pull requests and diffs in acs-tex — the IEEEtran LaTeX paper on speculative-execution attacks — against the rios0rios0/guide standards, with extra weight on the compilation cycle, bibliography integrity, and the Portuguese-first language setup. Use when reviewing a PR, a branch, or staged changes here."
---

# Code review — `acs-tex`

`acs-tex` holds an IEEE conference paper written in Brazilian Portuguese with `IEEEtran`, a BibTeX bibliography, and C code listings drawn from Spectre proofs of concept. There is no build system — the document is compiled by hand in four passes.
...
## What matters most in `acs-tex`

These are the checks that catch real defects in this repository. Work through
them before the generic ones.

- **The four-pass cycle is required**: `pdflatex` → `bibtex` → `pdflatex` → `pdflatex`. A pull request that changes citations or cross-references must have gone through it — a single pass leaves `??` markers and stale references in the PDF.
- **Every `\cite{}` needs an entry in `references.bib`**, and every entry should be cited. An undefined citation compiles with a warning and prints as `[?]`.
- **The document class is fixed**: `\documentclass[conference]{IEEEtran}`. Changing class or options silently reflows the whole paper and can break the submission format.
- **Language setup is Portuguese-first**: `\selectlanguage{brazil}` with English loaded as a secondary `babel` language, and UTF-8 input encoding. A change that reverses this breaks hyphenation and the auto-generated section labels.
- **Code listings use the shared style** loaded with `\usepackage{listings/c/style}`; new samples live under `listings/` and follow the same pattern rather than carrying inline `lstset` overrides.
- **The section order is the IEEE layout** — Introduction, Methodology, Results/Discussion, Conclusion, References. Reordering it is a structural change worth stating in the pull-request description.
- **Do not commit build artefacts** (`.aux`, `.log`, `.out`, `.bbl`, `.blg`, `.toc`), and check whether the generated PDF is meant to be committed before adding one.
- **Check the log for overfull and underfull boxes** on any change to figures, tables, or listings — a silently overflowing line is how a listing runs off the page in the printed proceedings.
- **The subject matter is published academic research.** Listings reproduce known public proofs of concept; keep the framing academic and attributed.
...
There is no test tooling. Verification is the full four-pass compile with a clean `.aux`, a read of the log for undefined references and overfull boxes, and a visual check of the affected pages in the PDF.
...
- The Portuguese prose and Portuguese section names — the paper's language is deliberate.
- The vendored `IEEEtran.cls` and `IEEEtran*.bst` files.
